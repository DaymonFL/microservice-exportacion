...
const storage = multer.diskStorage({
    destination: function(req, file, cb){
        ...
        const nombreFolder = `uploads/sesiones/sesion-${idSesionFile}/documentos-sesion`;
    ...
    filename: function(req, file, cb){
        ...
        if (file.mimetype == 'application/pdf'){
            const filename = `docSesion-distrito${distrito}.pdf`;
            const nombreFolder = `uploads/sesiones/sesion-${idSesionFile}/documentos-sesion/docSesion-distrito${distrito}.zip`;
            const nombreFolder2 = `uploads/sesiones/sesion-${idSesionFile}/documentos-sesion/docSesion-distrito${distrito}.pdf`;
            if(fs.existsSync(nombreFolder)){
            ...
            const filename = `docSesion-distrito${distrito}.zip`;
            const nombreFolder = `uploads/sesiones/sesion-${idSesionFile}/documentos-sesion/docSesion-distrito${distrito}.zip`;
            const nombreFolder2 = `uploads/sesiones/sesion-${idSesionFile}/documentos-sesion/docSesion-distrito${distrito}.pdf`;
            if(fs.existsSync(nombreFolder)){
                fs.unlinkSync(nombreFolder);
...
const middlewaresFileSesion = multer({storage: storage, limits: { fileSize: 10 * 1024 * 1024 }});

module.exports = middlewaresFileSesion;