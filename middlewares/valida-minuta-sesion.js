...
const storage = multer.diskStorage({
    destination: function(req, file, cb){
        ...
        const nombreFolder = `uploads/sesiones/sesion-${idSesionFile}/minutas-sesion`;
    ...
    filename: function(req, file, cb){
        ...
        if (file.mimetype == 'application/pdf'){
            const filename = `minutaSesion-distrito${distrito}.pdf`;
            const nombreFolder = `uploads/sesiones/sesion-${idSesionFile}/minutas-sesion/minutaSesion-distrito${distrito}.zip`;
            const nombreFolder2 = `uploads/sesiones/sesion-${idSesionFile}/minutas-sesion/minutaSesion-distrito${distrito}.pdf`;
            if(fs.existsSync(nombreFolder)){
            ...
            const filename = `minutaSesion-distrito${distrito}.zip`;
            const nombreFolder = `uploads/sesiones/sesion-${idSesionFile}/minutas-sesion/minutaSesion-distrito${distrito}.zip`;
            const nombreFolder2 = `uploads/sesiones/sesion-${idSesionFile}/minutas-sesion/minutaSesion-distrito${distrito}.pdf`;
            if(fs.existsSync(nombreFolder)){
                fs.unlinkSync(nombreFolder);
...
const middlewaresMinutaSesion = multer({storage: storage, limits: { fileSize: 10 * 1024 * 1024 }});

module.exports = middlewaresMinutaSesion;